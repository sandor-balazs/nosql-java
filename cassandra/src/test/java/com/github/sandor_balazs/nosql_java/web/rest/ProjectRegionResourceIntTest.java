...
@WebAppConfiguration
@IntegrationTest
public class ProjectRegionResourceIntTest extends AbstractCassandraTest {
    ...
    public void initTest() {
        projectRegionRepository.deleteAll();
    ...
    public void getProjectRegion() throws Exception {
        ...
        restProjectRegionMockMvc.perform(get("/api/projectRegions/{id}", projectRegion.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.id").value(projectRegion.getId().toString()));
    }

    @Test
    public void getNonExistingProjectRegion() throws Exception {
        // Get the projectRegion
        restProjectRegionMockMvc.perform(get("/api/projectRegions/{id}", UUID.randomUUID().toString()))

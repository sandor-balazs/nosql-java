...
@WebAppConfiguration
@IntegrationTest
public class SkillResourceIntTest extends AbstractCassandraTest {
    ...
    public void getAllSkills() throws Exception {
        ...
        restSkillMockMvc.perform(get("/api/skills?sort=id,desc"))
                ...
                .andExpect(jsonPath("$.[*].id").value(hasItem(skill.getId().toString())))
    ...
    public void getNonExistingSkill() throws Exception {
        // Get the skill
        restSkillMockMvc.perform(get("/api/skills/{id}", UUID.randomUUID().toString()))

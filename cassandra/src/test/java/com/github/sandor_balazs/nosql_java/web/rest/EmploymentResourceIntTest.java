...
@WebAppConfiguration
@IntegrationTest
public class EmploymentResourceIntTest extends AbstractCassandraTest {
    ...
    public void getAllEmployments() throws Exception {
        ...
        restEmploymentMockMvc.perform(get("/api/employments?sort=id,desc"))
                ...
                .andExpect(jsonPath("$.[*].id").value(hasItem(employment.getId().toString())))
                .andExpect(jsonPath("$.[*].fte").value(hasItem(DEFAULT_FTE.doubleValue())))
                .andExpect(jsonPath("$.[*].startDate").value(hasItem(DEFAULT_START_DATE.getTime())))
                .andExpect(jsonPath("$.[*].endDate").value(hasItem(DEFAULT_END_DATE.getTime())));
    }
    ...
    public void getEmployment() throws Exception {
        ...
        restEmploymentMockMvc.perform(get("/api/employments/{id}", employment.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.id").value(employment.getId().toString()))
            .andExpect(jsonPath("$.fte").value(DEFAULT_FTE.doubleValue()))
            .andExpect(jsonPath("$.startDate").value(DEFAULT_START_DATE.getTime()))
            .andExpect(jsonPath("$.endDate").value(DEFAULT_END_DATE.getTime()));
    }

    @Test
    public void getNonExistingEmployment() throws Exception {
        // Get the employment
        restEmploymentMockMvc.perform(get("/api/employments/{id}", UUID.randomUUID().toString()))

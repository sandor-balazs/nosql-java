...
@WebAppConfiguration
@IntegrationTest
public class AllocationResourceIntTest extends AbstractCassandraTest {
    ...
    public void getAllAllocations() throws Exception {
        ...
        restAllocationMockMvc.perform(get("/api/allocations?sort=id,desc"))
                ...
                .andExpect(jsonPath("$.[*].id").value(hasItem(allocation.getId().toString())))
    ...
    public void getAllocation() throws Exception {
        ...
        restAllocationMockMvc.perform(get("/api/allocations/{id}", allocation.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.id").value(allocation.getId().toString()))
    ...
    public void getNonExistingAllocation() throws Exception {
        // Get the allocation
        restAllocationMockMvc.perform(get("/api/allocations/{id}", UUID.randomUUID().toString()))
